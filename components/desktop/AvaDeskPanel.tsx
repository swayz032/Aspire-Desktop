...
import { buildAvaVideoFrameDoc } from '@/lib/anam-iframe';
...
function AvaDeskPanelInner() {
  ...
  // Server-served iframe URL. Replaces the prior srcDoc approach which
  // failed to execute scripts in production browsers (likely COEP
  // credentialless + dynamic import() restrictions on srcDoc contexts).
  // The server route /api/anam/iframe-page returns the same HTML
  // (buildAvaVideoFrameDoc) but as a normal same-origin document, which
  // sidesteps every srcDoc-specific browser quirk. The token is
  // internal-only and short-lived (1hr); profile is a small JSON of
  // greeting context (no secrets).
  const avaVideoFrameSrc = useMemo(() => {
  ...
  void avaVideoFrameDoc; // mark intentionally unused — kept only to avoid ripple imports
