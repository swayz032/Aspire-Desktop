/**
 * MemoryDetailRightRail — sticky right-side card stack on the memory detail
 * page.
 *
 * Composition rule (per plan §15.D):
 *   Always:      Details, Tags, Attachments & Activity (when files present).
 *   Conditional:
 *     - Participants     → meeting / call / session_summary / sms_thread
 *     - Linked Receipts  → invoice / quote / contract / session_summary
 *     - Linked Memories  → strategy / research / summary / thread_summary
 *
 * On web it's `position: sticky, top: 96` so the rail follows the viewport
 * as the center column scrolls. On native it stacks below the center column
 * (sticky positioning isn't supported in React Native by default).
 */
...
const PARTICIPANT_TYPES: MemoryType[] = ['meeting', 'call', 'session_summary', 'sms_thread', 'transcript'];
const RECEIPT_TYPES: MemoryType[] = ['invoice', 'quote', 'contract', 'session_summary'];
const LINKED_MEMORY_TYPES: MemoryType[] = ['strategy', 'research', 'summary', 'thread_summary', 'office_brief', 'finance_brief'];

export interface MemoryDetailRightRailProps {
  memory: MemoryDetail;
  /** Forwarded to inner cards. */
  onFilePress?: (fileId: string) => void;
  onLinkedMemoryPress?: (id: string) => void;
  onReceiptPress?: (id: string) => void;
}

export function MemoryDetailRightRail({
  memory,
  onFilePress,
  onLinkedMemoryPress,
  onReceiptPress,
}: MemoryDetailRightRailProps) {
  ...
  return (
    <View style={styles.rail} accessibilityLabel="Memory metadata">
      ...
      {items.length === 0 ? (
        ...
        <View style={cardStyles.list}>
          {items.map((item) => (
            ...
            >
              <View style={[cardStyles.linkIcon, { backgroundColor: hexA(tint, 0.10), borderColor: hexA(tint, 0.24) }]}>
...
export default MemoryDetailRightRail;
