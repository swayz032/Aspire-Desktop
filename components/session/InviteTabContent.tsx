/**
 * InviteTabContent — Reusable 4-tab invite system extracted from InviteSheet
 *
...
function injectInviteKeyframes() {
  if (Platform.OS !== 'web') return;
  if (document.getElementById('aspire-invite-keyframes')) return;
  const style = document.createElement('style');
  style.id = 'aspire-invite-keyframes';
  style.textContent = `
    @keyframes inviteRipple {
      0% { transform: scale(0.95); opacity: 1; box-shadow: 0 0 0 0 rgba(52, 199, 89, 0.4); }
      50% { transform: scale(1); box-shadow: 0 0 0 6px rgba(52, 199, 89, 0); }
      100% { transform: scale(1); opacity: 1; box-shadow: 0 0 0 0 rgba(52, 199, 89, 0); }
    }
    .invite-member-flash {
      animation: inviteRipple 0.6s ease-out;
    }
    .invite-search-input {
...
/**
 * StaggeredMemberRow — wraps a member card with FadeInDown entering animation.
 * Delay increases per index (Canvas.motion.stagger = 50ms base) for a cascading reveal.
 */
function StaggeredMemberRow({
  index,
  children,
}: {
  index: number;
  ...
  // Stagger delay: 50ms * index, capped at 250ms to avoid feeling sluggish
  const delay = Math.min(index * 50, 250);
  return (
    <Animated.View
      entering={FadeInDown.delay(delay).duration(200).springify().damping(18).stiffness(200)}
    >
      {children}
    </Animated.View>
  );
...
export function InviteTabContent({
  ...
  // Inject web-only CSS polish once
  useEffect(() => { injectInviteKeyframes(); }, []);
  ...
  }, [authenticatedFetch, correlationId]);
  ...
  }, [authenticatedFetch, roomName, roomLink, correlationId]);

  useEffect(() => {
    if (activeTab === 'link' && !roomLink) {
      generateRoomLink();
    }
  }, [activeTab, roomLink, generateRoomLink]);
  ...
  /** Reset all invite state — called by parent when closing */
  ...
  // Expose resetState via ref if needed (parent can call it)
  // For now, we expose it via the component instance key trick — parent remounts with key
  ...
  const memberListHeight = compact ? 200 : 280;
    ...
    <View>
      ...
      <View style={styles.tabs} accessibilityRole="tablist">
        ...
        {TABS.map((tab) => (
          <Pressable
            ...
            {...(Platform.OS === 'web' ? { className: 'invite-tab-btn' } as Record<string, string> : {})}
          >
            <Ionicons
              name={tab.icon}
              size={14}
      ...
      {activeTab === 'team' && (
        <View style={styles.content}>
          <View
            ...
            {...(Platform.OS === 'web' ? { className: 'invite-search-input' } as Record<string, string> : {})}
          >
          ...
          {!membersLoading && members.length > 0 && (
            <ScrollView style={{ maxHeight: memberListHeight }} showsVerticalScrollIndicator={false}>
              {members.map((member, index) => {
                const alreadyInvited = invitedIds.has(member.userId);
                const isFlashing = flashMemberId === member.userId;
                return (
                  <StaggeredMemberRow key={member.userId} index={index}>
                    <View
                      style={{ position: 'relative' }}
                      {...(Platform.OS === 'web' && isFlashing ? { className: 'invite-member-flash' } as Record<string, string> : {})}
                    >
                      {isFlashing && (
                        <Animated.View
      ...
      {activeTab === 'aspire' && (
        <View style={styles.content}>
          ...
          />
          <Text style={styles.formatHint}>The number shown on their Aspire profile</Text>
          ...
          />
          <Text style={styles.formatHint}>Their office code (e.g., A01)</Text>
          <PressableScale
            style={[
              styles.actionButton,
...
const styles = StyleSheet.create({
  ...
  // Success state (external invite) — subtle green glow on web
  successState: {
    alignItems: 'center',
    paddingVertical: Spacing.xxxl,
    gap: Spacing.md,
