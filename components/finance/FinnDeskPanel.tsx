...
function FinnDeskPanelInner({ initialTab, templateContext, isInOverlay, videoOnly, onEndCall }: FinnDeskPanelProps = {}) {
  ...
  const finnVoice = useAgentVoice({
    ...
    onError: (error) => {
      ...
      if (/auth_required/i.test(msg)) {
        ...
        showVoiceError('Finn Brain is warming back up. Try again in a few seconds.');
      } else if (/orchestrator_timeout|timeout/i.test(msg)) {
        showVoiceError('Finn took too long to respond. Please try again.');
  ...
  const onSend = async () => {
    ...
    try {
      // Fetch fresh financial context AND start SSE stream in parallel
      const [ctx, streamResp] = await Promise.all([
        fetchFinnContext(),
        fetch('/api/orchestrator/intent?stream=true', {
          method: 'POST',
          headers: {
      ...
      if (!streamResp.ok || !streamResp.body) {
        // Stream unavailable — fall back to local financial data
        ...
        const fallbackIntent = intent.includes('cash') || intent.includes('flow') ? 'cashflow'
          ...
          : 'default';
        const responseText = buildResponse(fallbackIntent, ctx.snapshot, ctx.exceptions);
        const now = Date.now();
        ...
        fallbackEvents.forEach((evt) => {
          setActiveRuns((prev) => {
            ...
            if (!run) return prev;
            return { ...prev, [runId]: { ...run, events: [...run.events, evt] } };
          });
        });
        setActiveRuns((prev) => {
          const run = prev[runId];
