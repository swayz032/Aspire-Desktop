/**
 * Service Memory — Results page.
 *
 * Mirror of `/office-memory/results` wired to the service-hub backend
 * (`useServiceMemorySearch`). URL state model identical: every filter is
 * read from useLocalSearchParams() and written back via router.setParams()
 * so the URL is shareable.
 *
 * Composition reuses 100% of the office-memory components: MemoryFilterBar,
 * MemoryGridListToggle, MemoryResultsGrid.
 */
...
import { useServiceMemorySearch } from '@/lib/service-memory/useServiceMemorySearch';
...
function ServiceMemoryResultsContent() {
  ...
  const result = useServiceMemorySearch(filters);
  ...
  const page = Math.max(
    1,
    Math.min(filters.page ?? 1, Math.max(1, Math.ceil(total / MEMORY_PAGE_SIZE))),
  );
  ...
  const handleFiltersChange = useCallback(
    (delta: Partial<MemoryFilters>) => {
      ...
      router.setParams(filtersToParams(next) as never);
    },
    [filters, router],
  );
  ...
  const handleViewChange = useCallback(
    (mode: MemoryViewMode) => {
      router.setParams({ view: mode === 'grid' ? '' : 'list' } as never);
    },
    [router],
  );
  ...
  const handlePageChange = useCallback(
    (n: number) => {
      router.setParams({ page: n > 1 ? String(n) : '' } as never);
    },
    [router],
  );
  ...
  const handleCardPress = useCallback(
    (memoryId: string) => {
      router.push(`/service-hub/memory/${memoryId}` as never);
    },
    [router],
  );
  ...
  const handleExport = useCallback(() => {
    devLog('[service-memory] export requested', { filters, total });
  }, [filters, total]);
    ...
    <View style={styles.page}>
      ...
      >
        <View style={styles.header}>
          <Text style={styles.title}>Service Memory Results</Text>
...
export default function ServiceMemoryResultsPage() {
  return (
    <PageErrorBoundary pageName="service-memory-results">
      <ServiceHubShell>
        <ServiceMemoryResultsContent />
      </ServiceHubShell>
    </PageErrorBoundary>
  );
}
...
const styles = StyleSheet.create({
  ...
  controlsLeft: { flex: 1, minWidth: 280 },
  controlsRight: { paddingTop: 0 },
  gridSection: { marginTop: 8, zIndex: 1 },
});
