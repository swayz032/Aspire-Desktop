...
const newPrompt = [
  ...
  '# Tone',
  '',
  'Speak in a friendly, confident, conversational manner.',
  '',
  ...
  '## invoke_adam',
  '',
  '- When to use: For live research, vendor lookups, and market data',
...
async function main() {
  ...
  if (r.status === 200) {
    ...
    console.log('Numbered steps:', numbered.length);
    console.log('');
    ...
    lines.forEach((l, i) => {
      if (l.startsWith('#') && i > 0 && lines[i-1].trim() !== '') {
        console.log('ISSUE: Line', i, '- heading missing blank line before:', l);
        issues++;
      }
    });
    if (issues === 0) console.log('No formatting issues found');
